...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($acao == 'criar') {
        $servico_model->categoria_id = $_POST['categoria_id'] ?? 1;
        $servico_model->nome = $_POST['nome'] ?? '';
        $servico_model->descricao = $_POST['descricao'] ?? '';
        $servico_model->preco_moto = $_POST['preco_moto'] ?? 0;
        $servico_model->preco_carro = $_POST['preco_carro'] ?? 0;
        $servico_model->preco_camioneta = $_POST['preco_camioneta'] ?? 0;
        $servico_model->duracao_minutos = $_POST['duracao_minutos'] ?? 60;
        $servico_model->estoque_disponivel = $_POST['estoque_disponivel'] ?? 999;
        $servico_model->ativo = $_POST['ativo'] ?? 1;
        
        if ($servico_model->create()) {
...
// Listar serviços
$stmt = $servico_model->readAllAdmin();
