...
SELECT 
    ...
    SUM(CASE WHEN a.status = 'pendente' THEN 1 ELSE 0 END) AS total_pendentes,
    SUM(a.valor_total) AS receita_total
FROM agendamentos a
WHERE a.data_agendamento >= (CURRENT_DATE - INTERVAL 6 DAY)
...
SELECT 
    s.id AS servico_id,
    s.nome,
    COUNT(ai.id) AS total_solicitacoes,
...
CREATE VIEW vw_dias_lotados AS
SELECT 
    data_agendamento AS dia,
    COUNT(*) AS total
FROM agendamentos
