...
- Página de embed no admin: `public/admin/metabase.php`
- Configure `METABASE_EMBED_URL` em `config/config.php:17` com o link público do seu dashboard.
- Teste rápido (apenas admin): `http://localhost/E-COMMERCE-main/public/admin/metabase.php?url=SEU_LINK_PUBLICO`

### Views de apoio no MySQL
- `vw_dashboard_semanal`: desempenho por dia da última semana
- `vw_servicos_mais_solicitados`: total de solicitações e receita por serviço
- `vw_dias_lotados`: dias com 9+ agendamentos ativos

##  Padrão MVC e Endpoints

- Lógica do Dashboard em `app/controllers/DashboardController.php`
- `public/admin/dashboard.php` consome dados via controller
- Endpoint de atualização de status: `public/api/mudar_status_agendamento.php`
